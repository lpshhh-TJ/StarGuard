...
{
    // [Fast Path] No Filter. Handle all IQs as they arrive.
    // osal_printk("recv_local_iq conn_id:%d\n", conn_id); // Reduced log
    ...
    if (report->report_idx + 1 == MEASURE_DIS_IQ_REPORT_CNT_MAX) {
        /* [Critical] Disable Print for Speed (0.6s -> 20ms) */
        // measure_dis_print_iq_data(0, 0, (sle_channel_sounding_iq_trans_t *)(&g_local_iq_data));
        
        // Notify State Machine (Legacy)
        measure_dis_client_set_iq_received();
        
        // [Fast Path] Imediately Send Response
        measure_dis_client_write_server(SLEM_PROFILE_MSG_IQ,
                                       (uint8_t *)&g_local_iq_data, 
