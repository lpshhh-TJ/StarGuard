...
void SystemClock_Config(void)
{
  ...
  */
  ...
  /* Activate I/O Compensation Cell for high-speed peripherals (LCD, QSPI, SDRAM) */
...
void MPU_Config(void)
{
  ...
  /* Configure SDRAM as Write-Through (required for LCD frame buffer) */
  MPU_InitStruct.Enable = MPU_REGION_ENABLE;
  ...
  /* Configure QSPI Flash as cached write-through.
     Try DTR + Cacheable: the cache-line fills do burst reads over QSPI.
     If this causes AHB deadlock, revert to non-cacheable.
     With cache enabled, warm inferences should be ~1s instead of ~7s. */
