...
class MultiScaleConv(nn.Cell):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # STM32 优化版：通道数大幅缩减，去掉 5×5 分支（2通道输入用大核收益低）
...
class Network(nn.Cell):
    def __init__(self):
        ...
        # 优化目标：~50M MAC（原 417M）
        # 策略：每层通道数缩减至 1/4 左右，去掉 5×5 分支
        self.features = nn.SequentialCell([

            # ===== Block 1 =====
            MultiScaleConv(in_channels=2, out_channels=12),  # -> 24 channels (原 96)
            nn.ReLU(),
            nn.Conv2d(24, 48, kernel_size=3, stride=1, pad_mode='same'),
            nn.BatchNorm2d(48),
            nn.ReLU(),
            ...
            # ===== Block 2 =====
            nn.Conv2d(48, 48, kernel_size=3, stride=1, pad_mode='same'),
            nn.BatchNorm2d(48),
            nn.ReLU(),

            nn.Conv2d(48, 64, kernel_size=3, stride=1, pad_mode='same'),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            ...
            # ===== Block 3 =====
            nn.Conv2d(64, 64, kernel_size=3, stride=1, pad_mode='same'),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        ])
